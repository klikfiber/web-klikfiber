...
const {mapMidtransStatus,acceptPaymentTransition} = load('lib/payments/midtrans-status.ts');
...
for (const status of ['pending','failed','cancelled','expired','paid']) assert.equal(acceptPaymentTransition('paid',status),false);
assert.equal(acceptPaymentTransition('paid','refunded'),true);
assert.equal(acceptPaymentTransition('paid','partially_refunded'),true);
assert.equal(acceptPaymentTransition('partially_refunded','paid'),false);
assert.equal(acceptPaymentTransition('refunded','paid'),false);
assert.equal(acceptPaymentTransition('expired','paid'),true);
assert.equal(acceptPaymentTransition('expired','pending'),false);
const {isStoreRoute} = load('lib/store-routes.ts');
for (const route of ['/','/produk','/akun/ringkasan','/akun/pesanan/order-id','/myshop','/sales','/produk/'+k33.id]) assert.ok(isStoreRoute(route),route);
assert.equal(isStoreRoute('/missing-page'),false);
assert.equal(isStoreRoute('/produk/not-a-product'),false);
console.log('PASS: duplicate/reordered payment notifications and public/private route recognition.');
