...
const withoutReferral = priceCart([{ id: k33.id, qty: 1 }], 'regular', null);
assert.equal(withoutReferral.discount, 0);
assert.equal(withoutReferral.total, k33.price + withoutReferral.shippingCost);
const withReferral = priceCart([{ id: k33.id, qty: 1 }], 'regular', { status:'active', percent:5, cap:300000, minSubtotal:0, used:0, reserved:0, budget:1000000, countUsed:0, countReserved:0, quota:10 });
assert.equal(withReferral.discount, 300000);
assert.equal(withReferral.total, withoutReferral.total - 300000);
